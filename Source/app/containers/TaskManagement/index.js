...
export default class TaskManagement extends Component
{
      ...
      <Animated.View style={[ styles.comming_region, {borderColor: color} ]} ref={stringConst.COMMING}
        onLayout={actions._set_region_layouts.bind(this, stringConst.COMMING)}>
          ...
          <View style={styles.comming_region_center} />
            <Icon onPress={actions._task_list_redirect.bind(this)} name='ios-more' style={styles.load_more_button} />
  ...
  _render_top_comming_tasks(comming_tasks = [])
  {
    ...
    //Lấy vị trí start_x
    let start_x = actions._calculate_start_x(top_comming_tasks.length, NORMAL_DISTANCE_X, NORMAL_SIZE);
    //Lấy vị trị start_y
    const OFFSET_TOP = 10;
  ...
  _render_bottom_comming_tasks(comming_tasks = [])
  {
    ...
    //Lấy vị trí start_x
    let start_x = actions._calculate_start_x(bottom_comming_tasks.length, NORMAL_DISTANCE_X, BIG_SIZE);
    //Lấy vị trị start_y
    const OFFSET_TOP = 5;
  ...
  _render_done_task()
  {
    ...
    let start_y = this.state.region_layouts[stringConst.DONE].y +
      this.state.region_layouts[stringConst.DONE].height / 2 - NORMAL_SIZE.height / 2;
